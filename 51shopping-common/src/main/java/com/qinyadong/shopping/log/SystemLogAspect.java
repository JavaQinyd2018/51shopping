...
public class SystemLogAspect {
    ...
    @Before("@annotation(com.qinyadong.shopping.log.SystemLog)")
    public void before(JoinPoint joinPoint) {
        SystemLog systemLog = getSystemLog(joinPoint);
        log.info("【{}】:请求参数是：【{}】",systemLog.value(),joinPoint.getArgs());
    }

    @AfterReturning(value = "@annotation(com.qinyadong.shopping.log.SystemLog)",returning = "result")
    public void afterReturning(JoinPoint joinPoint, Object result) {
        SystemLog systemLog = getSystemLog(joinPoint);
        log.info("【{}】:响应的结果是：【{}】",systemLog.value(),result);
    }

    @AfterThrowing(value = "@annotation(com.qinyadong.shopping.log.SystemLog)",throwing = "exception")
